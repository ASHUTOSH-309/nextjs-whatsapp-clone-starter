...
function Container({ data }) {
  ...
  const [publishStream, setPublishedStream] = useState(undefined)
  ...
  useEffect(() => {
    ...
    const startCall = async () => {
      ...
      import("zego-express-engine-webrtc").then(async ({ ZegoExpressEngine }) => {
        ...
        console.log("above zg in container")
        ...
        console.log(data)
        console.log("before room id line 103")
        await zg.loginRoom(`${data.roomID}`, token, { userID: userInfo.id.toString(), userName: userInfo.name }, { userUpdate: true })
  ...
  const endCall = () => {
    const id = data.id;
        ...
        if (zgVar && localStream && publishStream) {
              ...
              setZgVar.stopPublishingStream(publishStream)
  ...
  return <div className="border-conversation-border border-1 w-full bg-conversation-panel-background flex flex-col h-[100vh] overflow-hidden items-center justify-center text-white">
    ...
    <div className="flex flex-col gap-3 items-center">
      ...
      <span className="text-5xl">{data.name || data.from.name}</span>
...

    {
      ...
      (!callAccepted || data.callType === "audio") && <div className="my-24">
        {console.log(data)}
        {console.log("")}
        <Image src={data?.from?.profilePicture || data.profilePicture} alt="avatar" height={300} width={300} className="rounded-full" />
